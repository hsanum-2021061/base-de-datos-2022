...
-- -----------------------------------------------------
DROP DATABASE IF EXISTS db_control_ventas_henry_sanum;
CREATE DATABASE IF NOT EXISTS db_control_ventas_henry_sanum;
USE db_control_ventas_henry_sanum;
...
-- -----------------------------------------------------
drop procedure if exists p_ventas_create;
delimiter $$
...
BEGIN
	...
	productos.precio_mayor = (new.total/new.cantidad)+ ((NEW.total/NEW.cantidad)*0.15 )
    where
...
-- -------------------------------------------------------------------------
delimiter $$
drop trigger if exists tr_detalle_compras_before_insert $$
create trigger tr_detalle_compras_before_insert
after insert
on detalle_compras
for each row
begin
update productos set existencia = existencia + new.cantidad
where new.producto_id = productos.id;
end$$
delimiter ;
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
delimiter $$
drop trigger if exists tr_detalle_ventas_insert2 $$
create trigger tr_detalle_ventas_insert2
after insert
on detalle_ventas
for each row
begin
